<template>
    <div>
        <div v-if="showAlert" class="alert fixed flex p-4 mb-4 bg-red-100 rounded-lg dark:bg-red-200 mx-auto max-w-screen-xl t-0 r-0" role="alert">
            <svg class="flex-shrink-0 w-5 h-5 text-red-700 dark:text-red-800" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path></svg>
            <div class="ml-3 text-sm font-medium text-red-700 dark:text-red-800">
                Something went wrong try again please
            </div>
            <button @click="dismiss" type="button" class="ml-auto -mx-1.5 -my-1.5 bg-red-100 text-red-500 rounded-lg focus:ring-2 focus:ring-red-400 p-1.5 hover:bg-red-200 inline-flex h-8 w-8 dark:bg-red-200 dark:text-red-600 dark:hover:bg-red-300" data-dismiss-target="#alert-2" aria-label="Close">
                <span class="sr-only">Close</span>
                <svg  class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
            </button>
        </div>

        <div  v-if="showAlertSuccess" class="alert fixed flex p-4 mb-4 bg-green-100 rounded-lg dark:bg-green-200 mx-auto max-w-screen-xl t-0 r-0" role="alert">
            <svg class="flex-shrink-0 w-5 h-5 text-green-700 dark:text-green-800" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path></svg>
            <div class="ml-3 text-sm font-medium text-green-700 dark:text-green-800">
                Successfully send
            </div>
            <button @click="dismiss" type="button" class="ml-auto -mx-1.5 -my-1.5 bg-green-100 text-green-500 rounded-lg focus:ring-2 focus:ring-green-400 p-1.5 hover:bg-green-200 inline-flex h-8 w-8 dark:bg-green-200 dark:text-green-600 dark:hover:bg-green-300" data-dismiss-target="#alert-3" aria-label="Close">
                <span class="sr-only">Close</span>
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
            </button>
        </div>
        <div
            ...
            <div class="">
                <div>
                    ...
                    <input id="inputName" v-model="name" class="w-full bg-gray-200 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline" v-bind:class="errors.name !== null ? 'border border-red-500' :'' "
                ...
                <div class="mt-8">
                    ...
                    <input id="inputPhone" v-model="phone" class="w-full bg-gray-200 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline" v-bind:class="errors.phone !== null ? 'border border-red-500' :'' "
                           type="text" placeholder="+7-123-456-7890">
                    <p class="text-red-500 text-xs italic" v-if="errors.phone">{{errors.phone}}</p>
                </div>
                <div class="mt-8">
                    <span id="inputMessage" class="uppercase text-sm text-gray-600 font-bold">Message</span>
                    <textarea v-model="msg" v-bind:class="errors.msg !== null ? 'border border-red-500' :'' "
                              class="w-full h-32 bg-gray-200 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"></textarea>
                    <p class="text-red-500 text-xs italic" v-if="errors.msg">{{errors.msg}}</p>
                </div>
                <div class="mt-8">
                    <button @click="send"
    ...
    methods: {
        send(){
            ...
            if (this.name.length <= 1) {
                this.errors.name = "must be at least 2 letter"
            }else {
            ...
            if (this.msg.length <= 20) {
                this.errors.msg = "must be at least 20 letters"
            }else {
                this.errors.msg = null
            }

            this.phone = this.phone.trim()
            let r = /^(\+\d{1,2}-?)?\(?\d{3}\)?[\s.-]?\d{3}[\s.-]?\d{4}$/
            if (!r.test(this.phone) || this.phone.length < 11) {
                this.errors.phone = "must be in format: +7-123-456-7890 or +99-899-456-7890"
            }else {
            ...
            axios.post(route('api.contact.create'), {
                name: this.name,
                message: this.msg,
                phone: this.phone,
            }).then(value => {
